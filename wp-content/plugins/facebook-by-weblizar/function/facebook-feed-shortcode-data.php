...
//facebook page id
if(isset($facebook_feed_fetch["ffp_page_id"])){  $ffp_page_id=$facebook_feed_fetch["ffp_page_id"]; } else { $ffp_page_id="EAAETHMFXDyMBAJOSHSUgQrkC2y7aoTHKZCwr5hzZAUDjDWD36RqqdCg5sclue9SXIrh5HHkXuc5wV4MCSgcZBLMeWC5lZAQAqXZCFzxSZBSfDrAzFaAVCxgNPTQnA3QhrXbvJPednCma5IwcJPjlT1BacrQm1n0hgqkhylPwW4SgZDZD"; }
					    	...
					    	<?php } ?>
						       <!-- comment created time-->
							  	<div class="weblizar-fb-eapps-facebook-feed-item-date">
							  		<span><?php if(isset($data_content['comments']['data'][$c]['created_time'])) { $s = strtotime($data_content['comments']['data'][$c]['created_time']); printf( esc_html( _x( '%s Ago', '%s = human-readable time difference', WEBLIZAR_FACEBOOK_TEXT_DOMAIN ) ), human_time_diff( date( 'U', $s), current_time( 'timestamp' ) ) ); } ?> 
							  		</span>
							  	</div>
						    </div>
	                        <div class="weblizar-fb-eapps-facebook-feed-popup-item-content">
								<div class="weblizar-fb-eapps-facebook-feed-popup-item-content-text">
								 	<div class="weblizar-fb-eui-item-text">
										   ...
										   <!-- comment message-->
											<span class="webliza-r-fpcmt-dec"> <?php if(isset($data_content['comments']['data'][$c]['message'])) { echo esc_html( $data_content['comments']['data'][$c]['message'] ); } ?>  </span> 
											<!-- comment reply count-->
											<span class="webliza-r-fpcmt-reply"> <?php  if(isset($data_content['comments'][$c]['comment_count'])) {echo esc_html( $data_content['comments'][$c]['comment_count'] );} else { echo "0";}?>&nbsp;<?php esc_html_e('reply', WEBLIZAR_FACEBOOK_TEXT_DOMAIN);?>
											</span> 
									 	</div>
									</div>
							   </div>	
