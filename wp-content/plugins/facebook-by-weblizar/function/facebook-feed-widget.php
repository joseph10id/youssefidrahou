...
class Weblizar_facebook_feed_widget extends WP_Widget
{
						   ...
						   <!--header code cover image--> 
							...
							<div class="weblizar_fb-facebook-feed-top-area">
								<div class="weblizar_fb-facebook-feed-top-img ">
								 <!-- header link on logo --> 
									<a href="<?php if(isset($weblizar_header_data['link'])) { echo esc_url( $weblizar_header_data['link'] ); } else { echo "#";}?>" class="weblizar_fb-main-pic pull-left">
										<!-- header logo image --> 
								...
								<div class="weblizar_fb-facebook-feed-top-info col-md-9 col-sm-9 col-xs-9">
									<div class="weblizar_fb-facebook-feed-top-info-inner ">
										...
										<!-- header page name --> 
										<a class="fb_top_info_inner_header_link" href="<?php if(isset($weblizar_header_data['link'])) { echo esc_url( $weblizar_header_data['link'] ); } else { echo "#"; } ?>">  
											<?php if(isset($weblizar_header_data['name'])) { echo esc_html( $weblizar_header_data['name'] ); } ?> 
										</a>
										</h3>
										<!-- Like count --> 
									...
									<div class="weblizar_fb-facebook-feed-inner-text">
										<div class="button-group">
											<a type="button" target="_blank" class="weblizar_fb_btn_secondary feed_header_link" href="<?php if(isset($weblizar_header_data['link'])) { echo esc_url( $weblizar_header_data['link'] ); } else { echo "#";}?>"> <i class="fas fa-share"> </i> <?php esc_html_e('Share', WEBLIZAR_FACEBOOK_TEXT_DOMAIN);?> </a>
										</div>
								    </div>									
								</div>																	
							</div>
						    ...
						    <?php  if($weblizar_data_post["posts"]['data'][$y]['type']=='photo')
							        ...
							        <!-- post auther display -->
									    <div class=" col-md-12 box weblizar_fb-post_background_color">
										    ...
										    <div class="d-flex">
		                                        <!-- author picture-->										   
											    <div class="col-md-2 col-sm-2 col-3 weblizar_fb-logo-left"><img src="https://graph.facebook.com/<?php echo esc_attr($auher1['id']);?>/picture" class="img-responsive fb_logo_left" alt="img">
											    </div>
												<div class="col-md-10 col-sm-10 col-9 weblizar_fb-logo-right">
											        ...
											        <!-- post create time-->
												    <h6 class="weblizar_fb-post_font_color fb_create_time"><?php $s = strtotime($weblizar_data_post["posts"]['data'][$y]['created_time']); ?><i class="fas fa-clock-o"></i><span class="fb_date_size"><?php printf( esc_html( _x( '%s Ago', '%s = human-readable time difference', WEBLIZAR_FACEBOOK_TEXT_DOMAIN ) ), human_time_diff( date( 'U', $s), current_time( 'timestamp' ) ) ); ?><?php //echo date_i18n('F jS, g:i a',$s);?></span> </h6>
												</div>
											</div>
								        </div>
									...
									<div class="col-md-12 col-sm-12 col-xs-12">
									    ...
									    <?php } else { ?> <span class="weblizar_fb-post_font_color"> <?php echo esc_html( $text );?></span> <?php }  ?>
										</p>
									</div>  
					          <?php } ?>
						 ...
						 <?php  }  // post image data display end ?>
							    ...
							    <div class="col-md-12 col-sm-12 col-xs-12 bar weblizar_fb-post_background_color">
									...
									<div class=" weblizar_fb-bar-left">
										...
										<span class="share weblizar_fb-post_font_color"><a class="weblizar_fb-post_font_color" href="https://www.facebook.com/sharer/sharer.php?u=<?php if(isset($weblizar_data_post["posts"]['data'][$y]['link'])) { echo esc_url( $weblizar_data_post["posts"]['data'][$y]['link'] );} else { echo "#";}?>" target="_blank"><i class="fas fa-share"></i><?php  if($share !="") { echo esc_html( $share['count'] ); } else { echo "0";} ?></a></span>
										  <!-- post view on facebook display -->
										<span class="share weblizar_fb-post_font_color"><a href="https://www.facebook.com/<?php echo esc_attr( $id );?>" target="_blank" class="weblizar_fb-post_font_color"><i class="fas fa-eye"></i></a></span>
												   ...
												   <!--comment curl url -->
												    <?php $c_auther_url="https://graph.facebook.com/".$comment[$c]['id']."?access_token=".$token."";
														...
														<div class="col-md-2 col-sm-2 col-xs-2 weblizar_fb-post-comment-img">
														  <a href="<?php echo esc_url( "https://graph.facebook.com/".$c_auher1['from']['id'] );?>"><img src="https://graph.facebook.com/<?php echo esc_attr( $c_auher1['from']['id'] );?>/picture" class="img-responsive" /></a>
														</div>
													<?php } ?>
													 <!--comment author data -->
													<div class="col-md-10 col-sm-10 col-xs-10 weblizar_fb-post-comment-text-wrapper">
													    ...
													    <?php if(isset($c_auher1['from']['id'])) 
													        { ?>
															<a class="weblizar_fb-post_font_color" href="<?php echo esc_url( $c_auher1['from']['id'] );?>"><?php echo esc_html( $c_auher1['from']['name'] );?></a>
													   <?php } 
													  
															//comment message
															echo esc_html( $comment[$c]['message'] );?>
															</p>
															<span class="weblizar_fb-post_font_color"><?php esc_html_e('Posted:', WEBLIZAR_FACEBOOK_TEXT_DOMAIN);?> <?php $s = strtotime($comment[$c]['created_time']);printf( esc_html( _x( '%s Ago', '%s = human-readable time difference', WEBLIZAR_FACEBOOK_TEXT_DOMAIN ) ), human_time_diff( date( 'U', $s), current_time( 'timestamp' ) ) ); ?></span>
															<!--comment reply count-->
															<p class="weblizar_fb-post_font_color">
																<a href="" class="weblizar_fb-post_font_color"><i class="fas fa-reply"></i> <?php  if(isset($comment[$c]['comment_count'])) {echo esc_html( $comment[$c]['comment_count'] );} else { echo "0";}?><?php esc_html_e('Replies', WEBLIZAR_FACEBOOK_TEXT_DOMAIN);?></a>
															</p>
     ...
     */
	 public function form( $instance )
	 {
	 	...
	 	?>
	 	<p>
	 		<label for="<?php echo esc_attr( $this->get_field_id( 'Title' ) ); ?>"><?php esc_html_e( 'Widget Title', WEBLIZAR_FACEBOOK_TEXT_DOMAIN ); ?></label>
